...
public class SalesTerritoryControllerImp {
	...
	public String updateUser(@PathVariable("id") int id,
			@RequestParam(value = "action", required = true) String action,@ModelAttribute Salesterritory salesterritory, BindingResult bindingResult, Model model) {
		if (action != null && !action.equals("Cancel")) {
			if(bindingResult.hasErrors()) {
				model.addAttribute("territory", salesterritory);
				model.addAttribute("countries_region", countryRegionRepository.findAll());
				return "sales/territory/update-sales-territory";
			}
			try {
